...
A maintainer chooses when to publish by merging the Release pull request.
The only manual publication is a new plugin's first version, following the
one-time setup below. All later versions publish through the Release workflow.
...
configured. A maintainer publishes its first version manually, then configures
trusted publishing. This is the one-time exception to workflow-only publication;
it does not require a GitHub npm token secret or a separate workflow.

1. Merge the plugin pull request after its checks pass. It must include the
   package registration in `release-please-config.json`.
2. Wait for the Release workflow to prepare its Release pull request and mark
   it ready for review. Review the new plugin's version and changelog, then
   merge that Release pull request to approve the first release.
3. Let the Release run finish. Confirm that its build, package tests, tags, and
   GitHub Releases succeeded. The new plugin's npm publication is expected to
   fail authentication until this setup is complete. If the build failed, use
   the Skipped version recovery below instead of publishing that version.
4. In a clean checkout of that Release pull request's exact merge commit,
   install dependencies and build the package. Do not publish from the feature
   branch or a newer `main` commit. For live preview, run from the repo root:
   ...
   pnpm --filter @codlume/payload-live-preview publish --access public --no-git-checks
   ```

   Sign in with an npm account allowed to create packages under `@codlume` and
   complete npm's interactive authentication. Publish the version generated by
   release-please without editing it. `--no-git-checks` allows the detached
   checkout of the approved release commit.

5. On npm, open the new package's **Settings**, find **Trusted publishing**, and
...
6. Verify that `npm view @codlume/payload-live-preview version` matches the
   approved release. Once npm serves that version, use **Re-run failed jobs** on
   the Release run. It skips the version already published and finishes any
   remaining packages. This retry does not test the new trusted publisher for
   live preview; its next version's successful CI publication verifies that.

All subsequent versions use the existing Release pull request process, just
like activity and blurhash. If a temporary bootstrap token was already created,
revoke it in npm and delete the `NPM_BOOTSTRAP_TOKEN` GitHub secret.
...
approval step. Local publication is allowed only for the first-version setup
above, not for recovering later releases.
